...
function DimensionTest (dimension) {
  ...
  try {
    ...
    if ((playerdimcount / playercount) > 0.5) {
      ...
      world.getDimension('nether').runCommand(`event entity @e[type=entity:terminator] terminator:find_portal_to_${dim_output}`);
      world.getDimension('the end').runCommand(`event entity @e[type=entity:terminator] terminator:find_portal_to_${dim_output}`);
    };
  } catch (err) {
